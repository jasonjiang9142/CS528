"""
CS528 HW9 – HTTP client (same as HW4) for testing the web server on GKE.

Point --url at the external IP of the Kubernetes LoadBalancer service
(see: kubectl get svc hw9-server).

  # Hundreds of random page GETs from the bucket (run on a VM)
  python3 http_client.py --url http://EXTERNAL_IP:8080 --num 400 --delay 0.02

  # Single file
  python3 http_client.py --url http://EXTERNAL_IP:8080 --file pages/page_00001.json

  # Forbidden country (subscriber VM should print [Forbidden] ...)
  python3 http_client.py --url http://EXTERNAL_IP:8080 --file pages/page_00001.json --x-country Iran
"""
...
def main():
    ap = argparse.ArgumentParser(description="HW9 HTTP client")
    ap.add_argument("--url", required=True, help="Base URL  (e.g. http://LB_IP:8080)")
